...
export default function SignUpForm() {
  ...
  // this is used for verfying acc.
  const [verfying, setVerifying] = useState(false);
  const [isSubmiting, setIsSubmiting] = useState(false);
  const [verficationCode, setVerficationCode] = useState("");
  const [authError, setAuthError] = useState<string | null>(null);
  const [verificationError, setVerficationError] = useState<string | null>(
    null
  );
  ...
  const handleVerficationSubmit = async (
    ...
    try {
      ...
      if (result.status === "complete") {
        ...
        setVerficationError("Verfication could not be completed.");
      }
    } catch (error: any) {
          ...
          <form onSubmit={handleVerficationSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e: any) => setVerficationCode(e.target.value)}
                className="w-full"
                autoFocus
              />
            </div>
